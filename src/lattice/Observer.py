...
from consts import WINDOW_SIZE, MAX_CONCURRENT
...
class Observer:
    def __init__(self, name: str, model: AsyncLLM, format_model: SyncLLM, description: str = "the user's actions and screen activities"):
        self.name = name
        self.model = model
    ...
    async def make_session_observation(self, session: Interaction, observer_types: List[str] = ["default"]) -> List[dict | str]:
        ...
        for index in range(0, session_length, WINDOW_SIZE):
            sel_trace = session[index:index+WINDOW_SIZE]
            ...
            if "default" in observer_types:
                ...
                print("Other types of observers are not supported yet")
            tasks.append(self.model.call(fmt_prompt))
        return await batched_call(tasks, max_concurrent=MAX_CONCURRENT)

    async def make_observations(self, interactions: List[Session], observer_types: List[str] = ["default"]) -> List[List[dict]]: 
        """
        ...
        observations = await batched_call(tasks, max_concurrent=MAX_CONCURRENT)

        output_observations = []

        obs_id = 0
        for sid, session_observations in enumerate(observations):
            fmt_observations = [self._fmt_observation(observation) for observation in session_observations]
